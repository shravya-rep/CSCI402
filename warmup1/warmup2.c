...
double AvgPacketInterArrTime;

double totalPacketServiceTime;
double AvgPacketServiceTime;

double totalTimeSpentinQ1;
double AvgTimeSpentByPacketsInQ1;

double totalTimeSpentinQ2;
double AvgTimeSpentByPacketsInQ2;

double totalTimeSpentinS1;
double AvgTimeSpentByPacketsAtS1;

double totalTimeSpentinS2;
double AvgTimeSpentByPacketsAtS2;

double totalTimeSpentByPacketsInSystem;
double AvgTimeSpentByPacketsInSystem;
...
void AllocateValueForPacket(EachPacket* pointPacket)
{
    if (flag==0)// take default values
    {
        ...
        {
            pointPacket->serviceTime=10.0;
        }
        else
        {
            pointPacket->serviceTime=1000.0/mu;
        }
        
        //pointPacket->idealArrivalTime.tv_sec=1;
        //pointPacket->idealArrivalTime.tv_usec=0;
...
void AllocateValueForToken(struct timeval* interTokenTime)
{
    ...
    //(*interTokenTime).tv_sec=0;
    //(*interTokenTime).tv_usec=666667;
}

static 
void calAvgPacketInterArrTime(struct timeval difftime, int packetsTillNow)
{
    totalPacketInterArrTime+=printTimeInSec(difftime);
    AvgPacketInterArrTime=totalPacketInterArrTime/packetsTillNow;

}

static
void calAvgPacketServiceTime(struct timeval difftime, int packetCompleted)
{
    totalPacketServiceTime+=printTimeInSec(difftime);
    AvgPacketServiceTime=totalPacketServiceTime/packetCompleted;
}
...
static
void calAvgTimePacketInSystem(struct timeval difftime, int NoOfPacketsCompleted)
{
    totalTimeSpentByPacketsInSystem+=printTimeInSec(difftime);
    AvgTimeSpentByPacketsInSystem=totalTimeSpentByPacketsInSystem/NoOfPacketsCompleted;
}

static
void calStdDev(struct timeval difftime, int NoOfPacketsCompleted)
{
    TotalTimeSquare+=pow(printTimeInSec(difftime),2);
    double x2=TotalTimeSquare/NoOfPacketsCompleted;
    double var=x2-pow(AvgTimeSpentByPacketsInSystem,2);
...
static
void calAvgs(EachPacket* donePacket, int NoOfPacketsCompleted)
{
    struct timeval diff;
...
static 
void calTokenDropProb(int NoOfTokensDropped,int tokenNo)
{
...
static 
void calPacketDropProb(int NoOfPacketsDropped,int i)
{
    packetDropProb=(double)NoOfPacketsDropped/i;
}
...
{
    //struct timeval relstartTime={0,0};
    struct timeval sysTime;
    struct timeval relsysTime;
    struct timeval difftime;
    //struct timeval prevPacketArrTime=startTime;
    ...
    for(i=1;i<=numOfPackets;i++)
    {
        ...
        if(pointPacket->tokensNeeded<=B)
        {
            ...
            else
            {
                ...
                if(tokensInBucket>=pointPacket->tokensNeeded)
                {
                    ...
                    printf("%012.3fms: p%d enters Q2",printTime(pointPacket->q2EntryTime),pointPacket->pID);
        ...
        else
        {
            ...
            prevPacketArrTime=pointPacket->actualArrivalTime;  
            NoOfPacketsDropped++;
            calPacketDropProb(NoOfPacketsDropped,i);
...
void* token(void* arg)
{
    ...
    while(!ShouldTokenThreadTerminate())
    {
        ...
        else
        {
            ...
            NoOfTokensDropped++;
            calTokenDropProb(NoOfTokensDropped,tokenNo);
...
void* server(void* arg)
{
    ...
    while(!tokenThreadStopped||!My402ListEmpty(&Q2))
    {
        pthread_mutex_lock(&m);
    while(My402ListEmpty(&Q2)&&!Alldone)
    {
    ...
    else
    {
        ...
        calAvgPacketServiceTime(difftime,NoOfPacketsCompleted);
        calAvgTimePacketInSystem(timeinsys,NoOfPacketsCompleted);
        calStdDev(timeinsys,NoOfPacketsCompleted);
        calAvgs(firstInQ2Packet, NoOfPacketsCompleted);
        ...
        if(strcmp(argv[i],"-lambda")==0)
            {
                ...
                if(argc>=i+2)
                {
                    ...
                    {
                        fprintf(stderr,"\t(malformed command, value for \"-lambda\" is missing or incorrect. It must be + real no.) \n\t(Correct usage: warmup2 [-lambda lambda] [-mu mu] [-r r] [-B B] [-P P] [-n num] [-t tsfile])\n");
                        exit(1);
        ...
        else if(strcmp(argv[i],"-mu")==0)
            {
                ...
                if(argc>=i+2)
                {
                    ...
                    {
                        fprintf(stderr,"\t(malformed command, value for \"-mu\" is missing or incorrect. It must be + real no.)\n\t(Correct usage: warmup2 [-lambda lambda] [-mu mu] [-r r] [-B B] [-P P] [-n num] [-t tsfile])\n");
                        exit(1);
        ...
        else if(strcmp(argv[i],"-r")==0)
            {
                ...
                if(argc>=i+2)
                {
                    ...
                    {
                        fprintf(stderr,"\t(malformed command, value for \"-r\" is missing or incorrect. It must be + real no.)\n\t(Correct usage: warmup2 [-lambda lambda] [-mu mu] [-r r] [-B B] [-P P] [-n num] [-t tsfile])\n");
                        exit(1);
        ...
        else if(strcmp(argv[i],"-B")==0)
            {
                ...
                if(argc>=i+2)
                {
                    ...
                    {
                        fprintf(stderr,"\t(malformed command, value for \"-B\" is missing or incorrect. It must be a + integer.)\n\t(Correct usage: warmup2 [-lambda lambda] [-mu mu] [-r r] [-B B] [-P P] [-n num] [-t tsfile])\n");
                        exit(1);
        ...
        else if (strcmp(argv[i],"-P")==0)
            {
                ...
                if(argc>=i+2)
                {
                    ...
                    {
                        fprintf(stderr,"\t(malformed command, value for \"-P\" is missing or incorrect.)\n\t(Correct usage: warmup2 [-lambda lambda] [-mu mu] [-r r] [-B B] [-P P] [-n num] [-t tsfile])\n");
                        exit(1);
        ...
        else if(strcmp(argv[i],"-n")==0)
            {
                ...
                if(argc>=i+2)
                {
                    ...
                    {
                        fprintf(stderr,"\t(malformed command, value for \"-n\" is missing or incorrect.)\n\t(Correct usage: warmup2 [-lambda lambda] [-mu mu] [-r r] [-B B] [-P P] [-n num] [-t tsfile])\n");
                        exit(1);
